    ...
    <article className="rounded-[26px] border border-white/8 bg-black/8 p-5">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        ...
        <div className="rounded-[20px] bg-[color:var(--surface-low)] px-4 py-3 text-right">
          ...
          <div className="mt-2 text-sm font-medium">{formatUiDateTime(item.occurredAt)}</div>
        </div>
      </div>
    </article>
  );
  ...
  try {
    ...
    return (
      <ScreenState
        ...
        description="The main symptom feed hit a temporary read failure. You can still continue into the global intelligence workspace while the inbox data layer recovers."
        tone="error"
        actions={<Button size="lg" render={<Link href="/">Open global intelligence</Link>} />}
      />
    );
  }
          ...
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-3">
                <Button variant="outline" size="lg" render={<Link href="/">Open global intelligence</Link>} />
                <Button
                  variant="outline"
                  size="lg"
          ...
          <div className="space-y-6">
            <Card className="surface-panel rounded-[30px] px-0 py-0">
              <CardHeader className="px-6 pt-6">
                ...
                <CardTitle className="section-title mt-3">Triage-ready symptom stream</CardTitle>
              ...
              <CardContent className="px-4 pb-5 sm:px-5">
                <div className="grid gap-4 md:grid-cols-4">
                  <div className="rounded-[22px] bg-[color:var(--surface-low)] p-4">
                    ...
                    <div className="mt-2 text-2xl font-semibold">{inbox.counts.field_claim}</div>
                  </div>
                  <div className="rounded-[22px] bg-[color:var(--surface-low)] p-4">
                    <div className="eyebrow">Defects</div>
                    <div className="mt-2 text-2xl font-semibold">{inbox.counts.defect}</div>
                  </div>
                  <div className="rounded-[22px] bg-[color:var(--surface-low)] p-4">
                    <div className="eyebrow">Bad tests</div>
                    <div className="mt-2 text-2xl font-semibold">{inbox.counts.bad_test}</div>
                  </div>
                  <div className="rounded-[22px] bg-[color:var(--surface-low)] p-4">
                    <div className="eyebrow">Marginal</div>
                    <div className="mt-2 text-2xl font-semibold">{inbox.counts.marginal_test}</div>
                  </div>
                </div>
              </CardContent>
            </Card>
            ...
            <Card className="surface-sheet rounded-[30px] px-0 py-0">
              <CardHeader className="px-6 pt-6">
                <Badge>Inbox</Badge>
                <CardTitle className="section-title mt-3">Browse incoming signals</CardTitle>
          ...
          <div className="space-y-6">
            <Card className="surface-sheet rounded-[30px] px-0 py-0">
              ...
              <CardContent className="px-5 pb-5">
                <form className="space-y-4" action="/inbox">
